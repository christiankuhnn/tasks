...
export function findQuestion(
    ...
    return newq === -1 ? null : questions[newq];
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (sum: number, question: Question) =>
            sum + (question.published ? question.points : 0),
        0
    );
}
...
export function editOption(
    ...
    const index = questions.map((question: Question): Question => {
...
export function duplicateQuestionInArray(
    ...
    final.splice(index + 1, 0, duplicateQuestion(newId, questions[index]));
    return final;
}
